...
write (filename, '(A,A,A)' )'test_finiteT',trim(adjustl(size_str)),'.csv'
open(101,file=filename)
!set temperature points
do i=1,tpoints
 beta(i)=beta_min+dble(i-1)*beta_steps
 J=beta(i)
 call Initialise
 call MonteCarlo
...
do mciter=1,mcsteps
 ...
 if(mod(mciter,10)==0) then
  write(101,"(f20.10,<num_bonds>(',', I10))")J,(spins_x(ix,:size),ix=1,size),(spins_y(ix,:size),ix=1,size)
 end if
end do
...
do i=1,tpoints
 write(102,"(9(f20.10, ',', f20.10))")beta(i),en_per_site(i)/beta(i)
end do
close(102)
...
do i=1,tpoints
 write(103,"(9(f20.10,',', f20.10))")beta(i),C_v(i)
end do
close(103)
...
do i=1,tpoints
 write(104,"(f20.10, (',', f20.10))")beta(i),mag_per_site(i)
end do
close(104)
...
do i=1,tpoints
 write(105,"(9(f20.10,',', f20.10))")beta(i),xi(i)
end do
close(105)
